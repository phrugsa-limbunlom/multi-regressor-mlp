    ...
    def fit(self, x, y, epochs, batch_size):
        ...
        for epoch in range(epochs):
            ...
            for i in range(0, len(X_train_shuffled), batch_size):
                # if i == 0 or i == len(X_train_shuffled) - 1:
                # logging.info(f"Iteration {i + 1}")

                # feedforward
                hidden_layer_neurons = self.feedforward(X_train_shuffled[i:i + batch_size, :], self.weight_input_hidden,
                                                        self.bias_hidden)

                y_pred = self.feedforward(hidden_layer_neurons, self.weight_hidden_output, self.bias_output)

                training_loss_batch += self.loss.rmse(y_train_shuffled[i:i + batch_size], y_pred)

                # backward propagation on output to hidden layer
                gradient_output = self.gradient_output(y_train_shuffled[i:i + batch_size, :], y_pred)  # linear
                ...
                d_weight_input_hidden = self.calculate_dweight(gradient_hidden, X_train_shuffled[i:i + batch_size, :],
